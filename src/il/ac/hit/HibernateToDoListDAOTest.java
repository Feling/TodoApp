...
public class HibernateToDoListDAOTest {
	...
	public void testDeleteItem() {
		...
		//Prepare database - Create user and an item.
		User u = new User();
		u.setName("Sonic");
		u.setPassword("RemoveIt3m");
		Item i = new Item();
		i.setWhatToDo("Meet Tails and Knuckles for a game of rings.");
		i.setUser(u);
		try {
			model.addUser(u);
		...
		//Clean database - Delete users
		try{
			User sameTempUser = i.getUser();
			model.deleteUser(sameTempUser);
		}catch(ToDoListPlatformException e)
		{
			System.out.println("testRemoveItem : Unable to delete user.");
			e.printStackTrace();
		}
	}
